<?php
$app->post('/api/Kite/orderPhotobookByPDF', function ($request, $response, $args) {
    ...
    if (!empty($post_data['args']['insidePdf'])) {
        $post_data['args']['jobs'][0]['assets']['inside_pdf'] = $post_data['args']['insidePdf'];
    }
    if (!empty($post_data['args']['coverPdf'])) {
        $post_data['args']['jobs'][0]['assets']['cover_pdf'] = $post_data['args']['coverPdf'];
    }
