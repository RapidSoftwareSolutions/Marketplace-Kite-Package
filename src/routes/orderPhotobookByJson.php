<?php
$app->post('/api/Kite/orderPhotobookByJson', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'pages', 'customerPhone', 'templateType', 'recipientName', 'addressLine1', 'city', 'countryCode', 'postCode', 'amount', 'currency']);
    ...
    if (!empty($post_data['args']['frontCover'])) {
        $post_data['args']['jobs'][0]['assets']['front_cover'] = $post_data['args']['frontCover'];
    }
    if (!empty($post_data['args']['backCover'])) {
        $post_data['args']['jobs'][0]['assets']['back_cover'] = $post_data['args']['backCover'];
    }
    foreach ($post_data['args']['pages'] as $page){
        $post_data['args']['jobs'][0]['assets']['pages'][]=
            [
