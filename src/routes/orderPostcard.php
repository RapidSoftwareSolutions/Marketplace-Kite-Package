<?php
$app->post('/api/Kite/orderPostcard', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret','frontImage', 'customerPhone', 'recipientName', 'addressLine1', 'city', 'countryCode', 'postCode', 'amount', 'currency']);
    ...
    $post_data['args']['jobs'][0]['template_id'] = 'postcard';
    ...
    if (!empty($post_data['args']['message'])) {
        $post_data['args']['jobs'][0]['message'] = $post_data['args']['message'];
    }
