<?php
$app->post('/api/Kite/orderGreetingCard', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'frontImage', 'templateType', 'customerPhone', 'recipientName', 'addressLine1', 'city', 'countryCode', 'postCode', 'amount', 'currency']);
    ...
    $post_data['args']['jobs'][0]['assets']['front_image'] = $post_data['args']['frontImage'];
    if (!empty($post_data['args']['shippingClass'])) {
        $post_data['args']['jobs'][0]['shipping_class'] = $post_data['args']['shippingClass'];
    }

    if (!empty($post_data['args']['backImage'])) {
        $post_data['args']['jobs'][0]['back_image'] = $post_data['args']['backImage'];
    }
    if (!empty($post_data['args']['insideLeftImage'])) {
        $post_data['args']['jobs'][0]['inside_left_image'] = $post_data['args']['insideLeftImage'];
    }
   if (!empty($post_data['args']['insideRightImage'])) {
        $post_data['args']['jobs'][0]['inside_right_image'] = $post_data['args']['insideRightImage'];
    }
