<?php
$app->post('/api/Kite/orderDTGApparel', function ($request, $response, $args) {
    $settings = $this->settings;

    //checking properly formed json
    $checkRequest = $this->validation;
    $validateRes = $checkRequest->validate($request, ['apiKey', 'apiSecret', 'customerPhone', 'templateType', 'recipientName', 'addressLine1', 'city', 'countryCode', 'postCode', 'amount', 'currency']);
    ...
    if (!empty($post_data['args']['centerChest'])) {
        $post_data['args']['jobs'][0]['assets']['center_chest'] = $post_data['args']['centerChest'];
    }
    if (!empty($post_data['args']['centerBack'])) {
        $post_data['args']['jobs'][0]['assets']['center_back'] = $post_data['args']['centerBack'];
    }
    if (!empty($post_data['args']['garmentSize'])) {
        $post_data['args']['jobs'][0]['options']['garment_size'] = $post_data['args']['garmentSize'];
    }
    if (!empty($post_data['args']['garmentColor'])) {
        $post_data['args']['jobs'][0]['options']['garment_color'] = $post_data['args']['garmentColor'];
    }
